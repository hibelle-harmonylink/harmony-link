(() => {
  ...
  const dictionary = window.HARMONY_HANJA_DICTIONARY || { entries: legacyEntries, words: legacyWords.map(([hangul, hanja, reading, meaning]) => ({ hangul, hanja, reading, meaning })) };
  ...
  const renderResults = matches => {
    if (!matches.length) {
      results.innerHTML = '';
      status.textContent = '적절한 한자 변환 결과를 찾지 못했습니다.';
      return;
    }
    status.textContent = `${matches.length}개의 한자 후보를 찾았어요.`;
    results.innerHTML = `<h2 class="hanja-result-group">변환 결과</h2>${matches.map(item => resultCard(item)).join('')}`;
  };
  const search = () => {
    const value = query.value.trim();
    if (!value) {
      results.innerHTML = '';
      status.textContent = '한글을 입력해 주세요.';
      query.focus();
      return;
    }
    const exactWords = words.filter(word => word.hangul === value || word.hanja === value)
      .map(word => ({ character:word.hanja, reading:word.reading, meaning:word.meaning }));
    const relatedWords = exactWords.length || value.length < 2 ? [] : words
      .filter(word => word.hangul.includes(value) || word.hanja.includes(value))
      .map(word => ({ character:word.hanja, reading:word.reading, meaning:word.meaning }));
    const perCharacter = [...value].flatMap(character => {
      ...
      return value.length === 1 ? entries.filter(([, reading]) => reading === character).map(([hanja, reading, meaning]) => ({ character:hanja, reading, meaning })) : [];
    });
    const seen = new Set();
    renderResults([...exactWords, ...relatedWords, ...perCharacter].filter(item => {
