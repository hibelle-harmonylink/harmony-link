...
const partnerResourceSections = [
  ...
  {no:'11',tier:0,icon:'📌',title:'공지사항',copy:'매칭·행사·교육 및 시스템 소식',items:[['최신 공지사항 보기','community.html?refresh=20260805-265&category=notice','바로가기','view'],['파트너 공지사항 안내','downloads/HarmonyLink_Partner_Notice_Guide_v1.0.pdf','PDF'],['교육·행사 일정'],['자료실 업데이트']]},
  {no:'12',tier:0,icon:'💬',title:'커뮤니티',copy:'파트너끼리 경험과 정보를 나누는 공간',items:[['파트너 커뮤니티 입장','community.html?refresh=20260805-265','바로가기','view'],['커뮤니티 이용 가이드','downloads/HarmonyLink_Partner_Community_Guide_v1.0.pdf','PDF'],['질문·후기·자료 공유']]}
];
if (downloads) {
  const resourceMarkup = partnerResourceSections.map((section,index) => `<article class="partner-resource-group${section.premium?' premium-resource':''}" data-resource-tier="${section.tier}">
    ...
    <div class="partner-resource-items" id="partnerResource${index}"${index===0?'':' hidden'}>${section.items.map(item=>`<div class="partner-resource-item"><span>${item[2]||'준비 중'}</span><strong>${item[0]}</strong>${item[1]?(item[3]==='view'?`<a href="${item[1]}" target="_blank" rel="noopener">자료 보기 →</a>`:`<a href="${item[1]}" download>다운로드 ↓</a>`):'<small>자료 준비 중</small>'}</div>`).join('')}</div>
  </article>`).join('');
