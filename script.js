...
const adRooms={
  ...
  community:{ko:'협력 업체',en:'Community Partners',label:'COMMUNITY PARTNER',slots:4,items:[{name:'AALEAC',displayName:'Asian American Law Enforcement Advisory Council',copy:'홈페이지에서 기관 활동과 소식을 확인할 수 있습니다.',copyEn:'Visit the website to learn about the council and its community work.',url:'https://aaleac.org/',image:'assets/partners/aaleac-shield.png?v=20260720-34'}]},
...
document.querySelectorAll('[data-ad-room]').forEach(button=>button.addEventListener('click',()=>{
  ...
  const registeredCards=room.items.map(item=>`<article class="ad-partner-card">${item.imageOnly?`<button type="button" class="ad-partner-image ad-image-expand" data-image="${item.banner||item.image}" data-alt="${item.name} 광고 배너"><img src="${item.image}" alt="${item.name} 로고"><span>로고 클릭 · 배너 크게 보기</span></button>`:`<div class="ad-partner-image"><img src="${item.image}" alt="${item.displayName||item.name} logo"></div>`}<div class="ad-partner-copy"><span>${room.label}</span>${item.imageOnly?`<h3>${item.name}</h3>`:`<a class="ad-partner-name" href="${item.url}" target="_blank" rel="noopener noreferrer"><h3>${item.displayName||item.name}</h3></a>`}<p>${currentLanguage==='en'?(item.copyEn||item.copy):item.copy}</p>${item.imageOnly?`<div class="ad-partner-actions"><a href="${item.chatUrl}" target="_blank" rel="noopener noreferrer">오픈카톡 상담 ↗</a><a href="${item.brokerUrl}" target="_blank" rel="noopener noreferrer">브로커 페이지 ↗</a></div>`:''}</div></article>`).join('');
