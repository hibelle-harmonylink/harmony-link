...
if (oldSpecialtyStart) {
  ...
  specialtySection.querySelectorAll('[data-specialty]').forEach(button=>button.addEventListener('click',()=>{
    ...
    const teacherDescriptions={digital:{ko:'AI·스마트폰 활용을 일상에 바로 적용할 수 있도록 쉽고 친절하게 안내합니다.',en:'Friendly, practical guidance for applying AI and smartphone skills in everyday life.'},melody:{ko:'노래와 호흡, 음악 활동을 통해 즐거움과 따뜻한 공동체의 연결을 만듭니다.',en:'Creates joy and warm community connections through singing, breathing, and music.'}};
    const teacherRole=currentLanguage==='en'?program.teacherRoleEn:program.teacherRoleKo;
...
if(learnerDescription){learnerDescription.dataset.ko='교회, 학교, 도서관, 시니어센터, 기업, 커뮤니티부터<br class="mobile-only-break">친구 모임과 개인까지 원하는 장소에서 맞춤 교육을<br class="mobile-only-break">신청할 수 있습니다.';learnerDescription.dataset.en='Churches, schools, libraries, senior centers, companies, communities, small groups, and individuals<br>can request tailored learning wherever they are.';learnerDescription.innerHTML=learnerDescription.dataset[currentLanguage];}
...
if(refreshedContact){
  ...
  if(pageConsent){pageConsent.outerHTML=`<div class="contact-consent-list contact-form-wide"><label><input type="checkbox" name="개인정보 수집·이용 동의" value="동의" required><span data-ko="개인정보 수집에 동의합니다 (필수)" data-en="I agree to the collection of personal information (Required)">개인정보 수집에 동의합니다 (필수)</span><button type="button" class="consent-view" data-ko="보기" data-en="View">보기</button></label><label><input type="checkbox" name="마케팅 활용 동의" value="동의"><span data-ko="마케팅 활용 동의 (선택)" data-en="Marketing use consent (Optional)">마케팅 활용 동의 (선택)</span><button type="button" class="consent-view" data-ko="보기" data-en="View">보기</button></label></div>`;}
  const emailNote=refreshedContact.querySelector('.contact-info-card.email p');
  if(emailNote){emailNote.dataset.ko='언제든 문의를 남겨주시면<br class="mobile-only-break">확인 후 답변드립니다.';emailNote.dataset.en='Send an email anytime.<br class="mobile-only-break">We will respond after review.';}
  const responseTitle=refreshedContact.querySelector('.contact-info-card.response h3');
  if(responseTitle){responseTitle.dataset.ko='확인 후 담당자가 연락드립니다';responseTitle.dataset.en='Our Team Will Follow Up';}
  const responseNote=refreshedContact.querySelector('.contact-info-card.response p');
  if(responseNote){responseNote.dataset.ko='접수된 내용을 확인한 뒤<br class="mobile-only-break">이메일 또는 전화로 안내해 드립니다.';responseNote.dataset.en='After reviewing your inquiry,<br class="mobile-only-break">we will contact you by email or phone.';}
  ...
  const contactAutomationUrl='https://script.google.com/macros/s/AKfycbyhqA6SrlsPGy5EfbfAz8lPU7d6A-GdP7NGTWfuVaTdiW4b7faFbEM5Yc6yu_QgU19Y/exec';
  refreshedContact.querySelector('#contactPageForm')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget;const status=form.querySelector('.contact-page-status');const submit=form.querySelector('.contact-page-submit');status.textContent=currentLanguage==='en'?'Sending...':'전송 중입니다...';submit.disabled=true;const formData=new FormData(form);formData.set('접수 경로','Harmony Link 홈페이지');formData.set('접수 시각',new Date().toISOString());try{const deliveries=[fetch('https://formsubmit.co/ajax/hibelle@hibelleconsulting.com',{method:'POST',headers:{Accept:'application/json'},body:formData})];if(contactAutomationUrl)deliveries.push(fetch(contactAutomationUrl,{method:'POST',mode:'no-cors',body:formData}));const [emailResponse]=await Promise.all(deliveries);if(!emailResponse.ok)throw new Error('failed');status.textContent=currentLanguage==='en'?'Your inquiry has been submitted.':'문의가 접수되었습니다. 확인 후 연락드리겠습니다.';form.reset();}catch(error){status.textContent=currentLanguage==='en'?'Could not send. Please email us directly.':'전송하지 못했습니다. 이메일로 직접 문의해 주세요.';}finally{submit.disabled=false;}});
}
if(refreshedContact){
...
const openInquiryModal=(type='일반 문의')=>{
  inquiryModal.dataset.inquiryType=type;
...
partnerCenter.insertAdjacentHTML('beforeend', `<div class="container partner-upgrade-guide"><div class="partner-guide-head"><p class="eyebrow">PARTNER START GUIDE</p><h3 data-ko="로그인부터 파트너 등급 이용까지" data-en="From sign-in to partner access">로그인부터 파트너 등급 이용까지</h3><p data-ko="아래 순서대로 진행하면 신청과 승인 상태를 쉽게 확인할 수 있습니다." data-en="Follow these steps to apply, receive approval, and access your partner resources.">아래 순서대로 진행하면 신청과 승인 상태를 쉽게 확인할 수 있습니다.</p></div><ol><li><b>01</b><span data-ko="가입·로그인" data-en="Join or sign in">가입·로그인</span><small data-ko="Google 또는 카카오 계정으로 가입하고 로그인합니다." data-en="Join and sign in with Google or Kakao.">Google 또는 카카오 계정으로 가입하고 로그인합니다.</small></li><li><b>02</b><span data-ko="입점 파트너 신청" data-en="Partner application">입점 파트너 신청</span><small data-ko="업체 또는 개인 강사 신청서를 작성합니다." data-en="Complete the company or instructor application.">업체 또는 개인 강사 신청서를 작성합니다.</small></li><li><b>03</b><span data-ko="관리자 검토·승인" data-en="Review and approval">관리자 검토·승인</span><small data-ko="승인되면 무료 파트너 등급과 안내메일을 받습니다." data-en="Once approved, you receive Free Partner status and an email.">승인되면 무료 파트너 등급과 안내메일을 받습니다.</small></li><li><b>04</b><span data-ko="등급 선택·변경" data-en="Choose or upgrade tier">등급 선택·변경</span><small data-ko="$20 BASIC 또는 $50 PREMIUM을 문의하고 관리자가 등급을 변경합니다." data-en="Request $20 BASIC or $50 PREMIUM and the administrator updates your tier.">$20 BASIC 또는 $50 PREMIUM을 문의하고 관리자가 등급을 변경합니다.</small></li><li><b>05</b><span data-ko="다시 로그인·자료 이용" data-en="Sign in again and access">다시 로그인·자료 이용</span><small data-ko="로그아웃 후 다시 로그인하면 변경된 등급의 자료와 혜택이 표시됩니다." data-en="Sign out and back in to see resources and benefits for the new tier.">로그아웃 후 다시 로그인하면 변경된 등급의 자료와 혜택이 표시됩니다.</small></li></ol><div class="partner-guide-actions"><button type="button" class="btn btn-primary auth-open"><span data-ko="로그인하기" data-en="Sign In">로그인하기</span><b>→</b></button><a class="btn btn-outline" href="https://forms.gle/pF4xy5Jz4ycVouKo9" target="_blank" rel="noopener noreferrer"><span data-ko="파트너 신청서 작성" data-en="Apply as a Partner">파트너 신청서 작성</span><b>↗</b></a><a class="btn btn-outline" href="#contact"><span data-ko="등급 변경 문의" data-en="Ask About an Upgrade">등급 변경 문의</span><b>→</b></a></div></div>`);
...
const adRooms={
  ...
  community:{ko:'협력 업체',en:'Community Partners',label:'COMMUNITY PARTNER',slots:4,items:[{name:'AALEAC',displayNameKo:'아시안 아메리칸 사법 경찰자문위원회',displayNameEn:'Asian American Law Enforcement Advisory Council',copy:'아시안 커뮤니티와 사법기관의 소통과 협력을 지원합니다.<br>연락처 646-996-8093',copyEn:'Supporting communication and cooperation between<br>Asian American communities and law enforcement.<br>Contact 646-996-8093',url:'https://aaleac.org/',image:'assets/partners/aaleac-shield.png?v=20260720-46'}]},
...
const renderPromotionNews=()=>{
  ...
  media.dataset.item=news.image?.includes('hibelle-digital')?'digital':news.image?.includes('hibelle-online-english')?'english':news.titleKo==='AALEAC'?'aaleac':news.titleKo.includes('Yura Kim')?'yura':news.titleKo==='OrganicOne'?'organic':news.image?.includes('meeran-melody')?'melody':'';
...
document.querySelectorAll('[data-ad-room]').forEach(button=>button.addEventListener('click',()=>{
  ...
  adDirectoryModal.querySelectorAll('.ad-partner-card').forEach((card,index)=>{const item=room.items[index];if(item&&!item.imageOnly&&item.url){const actions=document.createElement('div');actions.className='ad-partner-actions';actions.innerHTML=`<a href="${item.url}" target="_blank" rel="noopener noreferrer" data-ko="공식 홈페이지 ↗" data-en="Official Website ↗">${currentLanguage==='en'?'Official Website ↗':'공식 홈페이지 ↗'}</a>`;card.querySelector('.ad-partner-copy')?.appendChild(actions);}});
  adDirectoryModal.querySelectorAll('.ad-image-expand').forEach(button=>button.addEventListener('click',()=>{const images=adImageModal.querySelectorAll('.ad-image-gallery img');images[0].src=button.dataset.image;images[0].alt=button.dataset.alt;const secondary=button.dataset.secondaryImage;if(secondary){images[1].src=secondary;images[1].alt=`${button.dataset.alt} 2`;images[1].hidden=false;}else{images[1].removeAttribute('src');images[1].hidden=true;}adImageModal.hidden=false;}));
...
learnerApplicationModal.innerHTML=`<div class="partner-modal-backdrop" data-learner-close></div><div class="partner-modal-panel"><div class="partner-modal-head"><div><p>HARMONY LINK LEARNING</p><h2 data-ko="교육 신청 안내" data-en="Learning Request">교육 신청 안내</h2></div><button class="partner-modal-close" type="button" data-learner-close aria-label="닫기">×</button></div><div class="partner-modal-scroll learner-flyer-scroll"><img class="learner-flyer-image" src="assets/partners/learner-application-20260718.png" alt="하모니링크 교육 신청 안내 전단지"></div><div class="partner-modal-actions"><p data-ko="교육 안내를 확인한 후 신청서를 작성해 주세요." data-en="Review the learning information, then complete the request form.">교육 안내를 확인한 후 신청서를 작성해 주세요.</p><a class="btn btn-primary" href="${requestFormUrl}" target="_blank" rel="noopener noreferrer"><span data-ko="교육 신청서 작성" data-en="Complete Learning Request">교육 신청서 작성</span><b>↗</b></a></div></div>`;
...
const partnerResourceSections = [
  ...
  {no:'11',tier:0,icon:'📌',title:'공지사항',copy:'매칭·행사·교육 및 시스템 소식',items:[['최신 공지사항 보기','https://hibelleharmony.com/community.html?refresh=20260812-297&category=notice','바로가기','view'],['파트너 공지사항 안내','downloads/HarmonyLink_Partner_Notice_Guide_v1.0.pdf','PDF'],['교육·행사 일정'],['자료실 업데이트']]},
  {no:'12',tier:0,icon:'💬',title:'커뮤니티',copy:'파트너끼리 경험과 정보를 나누는 공간',items:[['파트너 커뮤니티 입장','https://hibelleharmony.com/community.html?refresh=20260812-297','바로가기','view'],['커뮤니티 이용 가이드','downloads/HarmonyLink_Partner_Community_Guide_v1.0.pdf','PDF'],['질문·후기·자료 공유']]}
];
if (downloads) {
  const resourceMarkup = partnerResourceSections.map((section,index) => `<article class="partner-resource-group${section.premium?' premium-resource':''}" data-resource-tier="${section.tier}">
    <button type="button" class="partner-resource-toggle" aria-expanded="${index===0?'true':'false'}" aria-controls="partnerResource${index}"><span class="resource-number">${section.no}</span><b>${section.icon}</b><div><h3>${section.title}<em class="resource-tier-label">$${section.tier}</em></h3><p>${section.copy}</p></div><i>${index===0?'−':'＋'}</i></button>
    <div class="partner-resource-items" id="partnerResource${index}"${index===0?'':' hidden'}>${section.items.map(item=>`<div class="partner-resource-item"><span>${item[2]||'준비 중'}</span><strong>${item[0]}</strong>${item[1]?(item[3]==='view'?`<a href="${item[1]}">자료 보기 →</a>`:`<a href="${item[1]}" download>다운로드 ↓</a>`):'<small>자료 준비 중</small>'}</div>`).join('')}</div>
  </article>`).join('');
  downloads.innerHTML = `<div class="partner-library-head"><div class="partner-library-status"><span class="unlocked-badge">접근 승인됨</span><p>필요한 영역을 선택하면 다운로드 가능한 파일과 준비 중인 자료를 확인할 수 있습니다.</p></div><div class="partner-tier-guide" aria-label="파트너 등급"><button type="button" data-tier="0">$0 <b>무료</b></button><button type="button" data-tier="20">$20 <b>BASIC</b></button><button type="button" data-tier="50">$50 <b>PREMIUM</b></button></div><p class="partner-tier-benefit" aria-live="polite"></p></div><div class="partner-resource-library">${resourceMarkup}</div><p class="partner-download-warning">이 자료는 승인된 입점 파트너 전용입니다. 외부 공유 및 무단 배포를 금지합니다.</p>`;
  const benefitText={0:'$0 무료 파트너 · 시작 필수자료, 공지사항과 커뮤니티를 이용할 수 있습니다.',20:'$20 BASIC · 무료 혜택과 운영·수업·홍보·기관 제출 자료를 이용할 수 있습니다.',50:'$50 PREMIUM · 모든 자료와 우선 홍보·추천 노출 신청 혜택을 이용할 수 있습니다.'};
  const setAccessTier=(maxTier=0,selectedTier=maxTier)=>{const allowed=[0,20,50].filter(tier=>tier<=maxTier);const selected=allowed.includes(Number(selectedTier))?Number(selectedTier):Math.max(...allowed);downloads.querySelectorAll('.partner-tier-guide button').forEach(button=>{const tier=Number(button.dataset.tier);button.disabled=tier>maxTier;button.classList.toggle('active',tier===selected);button.setAttribute('aria-pressed',String(tier===selected));});downloads.querySelector('.partner-tier-benefit').textContent=benefitText[selected];downloads.querySelectorAll('[data-resource-tier]').forEach(section=>{section.hidden=Number(section.dataset.resourceTier)>selected;});};
