...
const renderPromotionNews=()=>{
  ...
  media.dataset.item=news.image?.includes('hibelle-digital')?'digital':news.image?.includes('hibelle-online-english')?'english':news.titleKo==='AALEAC'?'aaleac':news.titleKo.includes('Yura Kim')?'yura':news.titleKo==='OrganicOne'?'organic':news.image?.includes('meeran-melody')?'melody':news.image?.includes('jangsu-daycare')?'jangsu':'';
...
document.querySelectorAll('[data-ad-room]').forEach(button=>button.addEventListener('click',()=>{
  ...
  adDirectoryModal.querySelectorAll('.ad-partner-card').forEach((card,index)=>{const item=room.items[index];if(item&&!item.imageOnly&&item.url){const isJangsu=item.name==='Jangsu Daycare';const actionHref=isJangsu?'tel:+17187990133':item.url;const labelKo=isJangsu?'전화 바로걸기 ↗':'공식 홈페이지 ↗';const labelEn=isJangsu?'Call Now ↗':'Official Website ↗';const actions=document.createElement('div');actions.className='ad-partner-actions';actions.innerHTML=`<a href="${actionHref}" ${isJangsu?'':'target="_blank" rel="noopener noreferrer"'} data-ko="${labelKo}" data-en="${labelEn}">${currentLanguage==='en'?labelEn:labelKo}</a>`;card.querySelector('.ad-partner-copy')?.appendChild(actions);}});
...
const partnerResourceSections = [
  ...
  {no:'12',tier:0,icon:'💬',title:'커뮤니티',copy:'파트너끼리 경험과 정보를 나누는 공간',items:[['파트너 커뮤니티 입장','https://hibelleharmony.com/community.html?refresh=20260815-301','바로가기','view'],['커뮤니티 이용 가이드','downloads/HarmonyLink_Partner_Community_Guide_v1.0.pdf','PDF'],['질문·후기·자료 공유']]}
];
if (downloads) {
