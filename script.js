...
const renderPromotionNews=()=>{
  ...
  media.dataset.item=news.image?.includes('hibelle-digital')?'digital':news.image?.includes('hibelle-online-english')?'english':news.image?.includes('aaleac-shield')?'aaleac':news.titleKo.includes('Yura Kim')?'yura':news.titleKo==='OrganicOne'?'organic':news.image?.includes('meeran-melody')?'melody':news.image?.includes('jangsu-daycare')?'jangsu':'';
...
document.querySelectorAll('[data-ad-room]').forEach(button=>button.addEventListener('click',()=>{
  ...
  adDirectoryModal.querySelectorAll('.ad-partner-image[data-banner]').forEach(wrap=>wrap.addEventListener('click',()=>{const images=adImageModal.querySelectorAll('.ad-image-gallery img');images[0].src=wrap.dataset.banner;images[0].alt=wrap.querySelector('img')?.alt||'';images[1].removeAttribute('src');images[1].hidden=true;adImageModal.hidden=false;}));
