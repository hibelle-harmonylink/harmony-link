...
const renderPromotionNews=()=>{
  ...
  media.dataset.item=news.image?.includes('hibelle-digital')?'digital':news.image?.includes('hibelle-online-english')?'english':news.image?.includes('aaleac-shield')?'aaleac':news.titleKo.includes('Yura Kim')?'yura':news.image?.includes('organic-one')?'organic':news.image?.includes('meeran-melody')?'melody':news.image?.includes('jangsu-daycare')?'jangsu':'';
