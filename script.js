...
const adRooms={
  premium:{ko:'프리미엄 광고 업체',en:'Premium Advertisers',label:'PREMIUM ADVERTISER',slots:4,items:[{name:'Yura Kim · High Line Residential',copy:'배너 이미지를 클릭하면 크게 볼 수 있습니다. · 카카오톡 ID: hlresy',image:'assets/ads/highline-residential-ko.png',imageOnly:true}]},
...
document.querySelectorAll('[data-ad-room]').forEach(button=>button.addEventListener('click',()=>{
  ...
  const registeredCards=room.items.map(item=>`<article class="ad-partner-card">${item.imageOnly?`<button type="button" class="ad-partner-image ad-image-expand" data-image="${item.image}" data-alt="${item.name} 광고 배너"><img src="${item.image}" alt="${item.name} 광고 배너"><span>클릭해서 크게 보기</span></button>`:`<div class="ad-partner-image"><img src="${item.image}" alt="${item.name} 로고 및 기관명"></div>`}<div class="ad-partner-copy"><span>${room.label}</span>${item.imageOnly?`<h3>${item.name}</h3>`:`<a class="ad-partner-name" href="${item.url}" target="_blank" rel="noopener noreferrer"><h3>${item.name} <b>↗</b></h3></a>`}<p>${item.copy}</p></div></article>`).join('');
