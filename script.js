...
const businessSpotlights = [
  ...
  {region:'ny',item:{...adRooms.community.items[1],url:'',summaryKo:'시니어를 위한 데이케어 서비스',summaryEn:'Daycare services for seniors'},categoryKo:'시니어 케어',categoryEn:'Senior Care',locationKo:'Flushing, New York',locationEn:'Flushing, New York',address:'32-38 148th St, Flushing, NY 11354',flyers:['assets/partners/jangsu-daycare-banner.png']},
  {region:'tx',item:dmsCareBusiness,categoryKo:'케어 전문 교육센터',categoryEn:'Care Training Center',locationKo:'Texas',locationEn:'Texas',address:'1933 E Frankford Rd. Suite 165, Carrollton, TX 75007',flyers:['assets/images/dms-care-flyer-en.png']}
];
...
const renderBusinessSpotlights = (selectedRegion='all') => {
  ...
  if(!businesses.length){
    ...
    grid.innerHTML=businesses.map((business,index)=>{const {region,item,categoryKo,categoryEn,locationKo,locationEn,address,mapQuery}=business;
      ...
      const addressMarkup=address?`<a class="business-address" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mapQuery||address)}" target="_blank" rel="noopener noreferrer">${address}</a>`:`<p class="business-address" data-ko="${locationKo}" data-en="${locationEn}">${displayLocation}</p>`;
