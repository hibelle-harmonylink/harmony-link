...
if (oldSpecialtyStart) {
  ...
  specialtySection.innerHTML = `<div class="container"><div class="section-heading centered reveal"><p class="eyebrow">PREMIUM SPECIALTY PROGRAMS</p><h2 data-ko="전문 교육 프로그램" data-en="Specialty Programs">전문 교육 프로그램</h2><p data-ko="Harmony Link가 직접 운영하거나 공동으로 운영하는 전문 프로그램을 만나보세요." data-en="Explore specialty programs operated directly or jointly by Harmony Link.">Harmony Link가 직접 운영하거나 공동으로 운영하는 전문 프로그램을 만나보세요.</p></div><div class="specialty-banner-grid">${specialtyPrograms.map((program,index)=>{
    const actionHtml = program.id === 'digital'
      ? `<div class="specialty-btn-row"><button type="button" class="btn specialty-open" data-specialty="${program.id}"><span data-ko="강사 소개" data-en="Meet the Instructor">강사 소개</span><b>↗</b></button><a class="btn btn-outline specialty-programs-link" href="digital-classes/index.html"><span data-ko="프로그램 보기" data-en="View Programs">프로그램 보기</span><b>→</b></a></div>`
...
const openAdGallery=(sources,alt)=>{const gallery=adImageModal.querySelector('.ad-image-gallery');gallery.innerHTML=sources.filter(Boolean).map((source,index)=>`<img src="${source}" alt="${alt}${index?` ${index+1}`:''}">`).join('');adImageModal.hidden=false;};
...
const adRooms={
  premium:{ko:'프리미엄 광고 업체',en:'Premium Advertisers',label:'PREMIUM ADVERTISER',slots:4,items:[{name:'Yura Kim · High Line Residential',copy:'뉴욕 부동산 에이전트 유라 김<br>좋은 매물 선정부터 계약과 입주까지 함께합니다.<br>연락처 516-390-1383',copyEn:'New York real estate agent Yura Kim<br>Property selection through closing and move-in.<br>Contact 516-390-1383',image:'assets/ads/highline-hl-symbol.png?v=20260819-3',banners:['assets/ads/highline-residential-ko.png'],chatUrl:'https://open.kakao.com/me/hlresy',brokerUrl:'https://www.hlres.com/yura-kim',imageOnly:true},{name:'OrganicOne',displayNameKo:'올가닉 원 유기농원',displayNameEn:'OrganicOne',copy:'유아동 식품·유기농 건강식품·한국 전통 식품을<br>팰팍과 플러싱 매장에서 만나보세요.<br>연락처 팰팍 201-585-0958 · 플러싱 929-845-0958',copyEn:'Organic children’s foods, health foods, and Korean traditional foods.<br>Available in Palisades Park and Flushing.<br>Contact<br>Palisades Park 201-585-0958 / Flushing 929-845-0958',image:'assets/ads/organic-one/logo-green.png',banners:['assets/ads/organic-one/family-products.png','assets/ads/organic-one/traditional-foods.png'],chatUrl:'https://www.instagram.com/organicone_/',brokerUrl:'https://www.organiconestore.com/',imageOnly:true},{name:'HOLE19 Golf Lounge',displayNameKo:'HOLE19 골프라운지',displayNameEn:'HOLE19 Golf Lounge',copy:'최신 골프 시뮬레이터와 커피·맥주·음식을 함께 즐기는 골프라운지입니다.<br>연락처 929-766-0088',copyEn:'A premium golf lounge with modern simulators, coffee, beer, and food.<br>Contact 929-766-0088',image:'assets/ads/hole19/logo.png?v=20260819-2',banners:['assets/ads/hole19/open-banner.png','assets/ads/hole19/promotion.png','assets/ads/hole19/features.png'],chatUrl:'tel:+19297660088',brokerUrl:'https://www.google.com/maps/search/?api=1&query=154-05%20Northern%20Blvd%2C%202F%2C%20Flushing%2C%20NY%2011354',primaryLabelKo:'전화 연결 ↗',primaryLabelEn:'Call Now ↗',secondaryLabelKo:'지도에서 보기 ↗',secondaryLabelEn:'View Map ↗',imageOnly:true}]},
...
document.querySelectorAll('[data-ad-room]').forEach(button=>button.addEventListener('click',()=>{
  ...
  adDirectoryModal.querySelectorAll('.ad-image-expand').forEach(button=>button.addEventListener('click',()=>openAdGallery((button.dataset.images||'').split('|'),button.dataset.alt)));
  adDirectoryModal.querySelectorAll('.ad-partner-image[data-banner]').forEach(wrap=>wrap.addEventListener('click',()=>openAdGallery([wrap.dataset.banner],wrap.querySelector('img')?.alt||'')));
