...
test('current admin metadata RPC remains untouched while business spotlight stays separate from partner access', () => {
  ...
  assert.match(homepageScript, /assets\/images\/dms-care-flyer-ko\.png/);
  ...
  assert.match(homepageScript, /flyerLabels:\['제품 안내','전통식품'\]/);
  assert.match(homepageScript, /flyerLabels:\['오픈 안내','프로모션','시설 안내'\]/);
  assert.match(homepageScript, /flyerLabels:\['한글 전단지','English Flyer'\]/);
  assert.match(homepageScript, /switcher\.hidden=flyers\.length<2/);
  ...
  assert.match(homepageStyles, /\.business-flyer-switcher\{display:flex;flex-wrap:wrap/);
});
