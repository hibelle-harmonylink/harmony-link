...
test('application-synced metadata fields stay editable without the direct-management border', () => {
  ['detailFullName', 'detailPhone', 'detailSpecialty', 'detailTeachingSubjects', 'detailEnrolledSubject', 'detailAssignedInstructor'].forEach(id => assert.match(admin, new RegExp(String.raw`\$\{syncedFieldClass\}"[^>]*>[^<]*<input id="${id}"`)));
  assert.match(css, /\.field-source-sync>input,\.field-source-sync>select\{border:1px solid #c8d8ea/);
...
test('operational settings and automatic system fields remain visually and functionally distinct', () => {
  ...
  assert.match(admin, /p_full_name: metadata\.fullName/);
});

test('source labels preserve the compact no-scroll and mobile width protections', () => {
  assert.match(admin, /title="신청서 자동연동 · 관리자 수정 가능"/);
