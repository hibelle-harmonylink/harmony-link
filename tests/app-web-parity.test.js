...
const serviceWorker = read('app/service-worker-v102.js');
...
test('service worker v102 precaches the HOME CTA/titles pass without changing the caching strategy', () => {
  assert.match(serviceWorker, /const CACHE="harmony-link-app-v102"/);
  ...
  assert.match(serviceWorker, /"\.\/overrides\.css\?v=102"/);
  assert.match(serviceWorker, /"\.\/app\.js\?v=102"/);
  // Same network-first, cache-as-fallback strategy as v101 -- not rewritten.
  assert.match(serviceWorker, /fetch\(event\.request,\{cache:"no-store"\}\)/);
  assert.match(appScript, /register\("service-worker-v102\.js",\{updateViaCache:"none"\}\)/);
