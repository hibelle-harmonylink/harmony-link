-- Finalize authenticated-only community writes while preserving public reads.
begin;

do $$
begin
  if to_regprocedure('public.create_public_community_post(text,text,text,text,text,text)') is not null then
    execute 'revoke execute on function public.create_public_community_post(text, text, text, text, text, text) from anon, authenticated';
  end if;
end;
$$;

-- Deliberately omit CASCADE: unexpected external dependencies must stop this migration.
