-- Re-assert (idempotent create-or-replace, no-op if already current) the
-- fix from 202608100003_admin_membership_type_fix.sql, which allows
-- admin_update_member_type() to set a member's type to 'partner' and
-- stopped the role->member_type trigger from silently overriding a
-- just-saved member_type. If that earlier migration was for any reason
-- never applied in this environment, admin_update_member_type(id,
-- 'partner') would still be rejected today with "invalid member type",
-- which is exactly the kind of RPC/front-end parameter mismatch that can
-- make "일반회원 -> 파트너" saves only partially take effect. Running this
-- migration guarantees the corrected definitions are live regardless of
-- prior migration history. No table structure changes; safe to re-run.
begin;
